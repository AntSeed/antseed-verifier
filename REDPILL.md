...
Start the seller with `--verifiers antseed-verifier`. The ACI adapter runs in-process — no
separate shim. Select it with `ANTSEED_VERIFIER_PROVIDER_ADAPTER=aci`.
...
The adapter fetches the nonce-bound ACI report, extracts the TDX quote, and binds it with
...
Run the seller inside the gateway's dstack CVM and mint the node quote via the dstack source:
